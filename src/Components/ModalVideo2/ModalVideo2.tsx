...
export default function ModalVideo2() {
  const { isOpen, onOpen, onClose } = useDisclosure();
  const [imageVideo2, setImageVideo2]= useState("");
  const [tituloVideo2, setTituloVideo2] = useState("");
  const [conteudoVideo2, setConteudoVideo2] = useState("");
  const [pasteLinkVideo2, setPasteLinkVideo2] = useState("");
  const {URLPutVideo2, setURLPutVideo2} = useContext(AppContext);
  ...
  async function createVideo2() {
    try {
      if (tituloVideo2 === '' || conteudoVideo2 === '' || pasteLinkVideo2 === '') {
        toast({
          title: "Erro",
      ...
      fetch(`${URLPutVideo2}`, {
        ...
        body: JSON.stringify({
          TITULO: tituloVideo2,
          CONTEUDO: conteudoVideo2,
          LINK: pasteLinkVideo2, 
          IMG2: imageVideo2,
        }),
      ...
      if (data.message.includes("Ok")) {
        onClose() 
          toast({
            title: "Sucesso",
            description: "Video 2 criado com sucesso!",
            status: "success",
            duration: 6000,
            isClosable: true,
            position: "bottom-left",
  ...
  const downImage = (imageFile: any) => {
    ...
    getDownloadURL(storageRef)
      .then((url2) => {
        ...
        xhr.send();
        setImageVideo2(url2);
        console.log("Verificando o valor de estado da imagem " , imageVideo2);
        ...
        <ModalContent>
          ...
          <ModalBody pb={6}>
            <FormControl>
              ...
              <Input value={tituloVideo2} onChange={(e) => setTituloVideo2(e.target.value)} focusBorderColor="black" ref={initialRef} placeholder="Digite aqui o título do Video" />
            </FormControl>
            <FormControl>
              <FormLabel>Cole aqui o Link do Video</FormLabel>
              <Input value={pasteLinkVideo2} onChange={(e) => setPasteLinkVideo2(e.target.value)} focusBorderColor="black" ref={initialRef} placeholder="Cole aqui o Link do Video" />
            </FormControl>
            <FormControl style={{visibility: 'initial'}} >
              <FormLabel>Conteúdo</FormLabel>
              <Textarea  value={conteudoVideo2} style={{marginBottom: "2vw"}} size={'lg'} onChange={(e) => setConteudoVideo2(e.target.value)}  focusBorderColor="black" ref={initialRef} placeholder="Digite todo o conteúdo do Video aqui!" />
            </FormControl>
            ...
            >
              ...
              <label htmlFor="file-input">
                ...
                height={320}
                  ...
                  // name="profile-image"
                  src={imageVideo2 || "/img-sec-banner.png"}
                  cursor="pointer"
                />
              </label>
