...
export default function ModalNotice2() {
  const { isOpen, onOpen, onClose } = useDisclosure();
  const [imageNotice2, setImageNotice2]= useState("");
  const [tituloNotice2, setTituloNotice2] = useState("");
  const [conteudoNotice2, setConteudoNotice2] = useState("");
  const {URLPutNotice2, setURLPutNotice2} = useContext(AppContext);
  ...
  async function createNotice2() {
    try {
      if (tituloNotice2 === '' || conteudoNotice2 === '') {
        toast({
          title: "Erro",
      ...
      fetch(`${URLPutNotice2}`, {
        ...
        method: "PUT",
        body: JSON.stringify({
          TITULO: tituloNotice2,
          CONTEUDO: conteudoNotice2,
          IMG2: imageNotice2,
        }),
      ...
      if (data.message.includes("Ok")) {
        onClose() 
          toast({
            title: "Sucesso",
            description: "Notícia 2 criada com sucesso!",
            status: "success",
            duration: 6000,
            isClosable: true,
            position: "bottom-left",
  ...
  const downImage = (imageFile: any) => {
    ...
    getDownloadURL(storageRef)
      .then((url2) => {
        ...
        xhr.open('GET', url2);
        xhr.send();
        setImageNotice2(url2);
        console.log("Verificando o valor de estado da imagem " , imageNotice2);
        ...
        <ModalContent>
          ...
          <ModalBody pb={6}>
            <FormControl>
              ...
              <Input value={tituloNotice2} onChange={(e) => setTituloNotice2(e.target.value)} focusBorderColor="black" ref={initialRef} placeholder="Digite aqui o título da notícia" />
            </FormControl>
            <FormControl style={{visibility: 'initial'}} >
              <FormLabel>Conteúdo</FormLabel>
              <Textarea  value={conteudoNotice2} style={{marginBottom: "2vw"}} size={'lg'} onChange={(e) => setConteudoNotice2(e.target.value)}  focusBorderColor="black" ref={initialRef} placeholder="Digite todo o conteúdo da Notícia aqui!" />
            </FormControl>
            ...
            >
              ...
              <label htmlFor="file-input">
                ...
                height={320}
                  ...
                  // name="profile-image"
                  src={imageNotice2 || "/img-sec-banner.png"}
                  cursor="pointer"
                />
              </label>
