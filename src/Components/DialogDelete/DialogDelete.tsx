...
export default function DialogDelete({ id, Close }: Props) {
  const { isOpen, onOpen, onClose } = useDisclosure();
  const cancelRef = useRef();
  const toast = useToast()

  const {URLDeleteArtigo1, setURLDeleArtigo1}= useContext(AppContext);

  async function deleteArtigo1() {
    try {
      
      
      const response = await 
      // `https://boxer-relieved-impala.ngrok-free.app/allDiaconato/upArtigo`
      fetch(`${URLDeleteArtigo1}`, {
        headers: {
