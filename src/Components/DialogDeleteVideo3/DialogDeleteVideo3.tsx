...
export default function DialogDeleteVideo3({ id, Close }: Props) {
  const { isOpen, onOpen, onClose } = useDisclosure();
  const cancelRef = useRef();
  const toast = useToast()

  const {URLDeleteVideo3, setURLDeleteVideo3}= useContext(AppContext);

  async function deleteVideo3() {
    try {
      
      
      const response = await 
      // `https://boxer-relieved-impala.ngrok-free.app/allDiaconato/upArtigo`
      fetch(`${URLDeleteVideo3}`, {
        headers: {
