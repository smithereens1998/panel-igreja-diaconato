...
export default function DialogDelete3({ id, Close }: Props) {
  const { isOpen, onOpen, onClose } = useDisclosure();
  const cancelRef = useRef();
  const toast = useToast()

  const {URLDeleteArtigo3, setURLDeleArtigo3}= useContext(AppContext);

  async function deleteArtigo3() {
    try {
      
      
      const response = await 
      // `https://boxer-relieved-impala.ngrok-free.app/allDiaconato/upArtigo`
      fetch(`${URLDeleteArtigo3}`, {
        headers: {
