...
export default function DialogDelete2({ id, Close }: Props) {
  const { isOpen, onOpen, onClose } = useDisclosure();
  const cancelRef = useRef();
  const toast = useToast()

  const {URLDeleteArtigo2, setURLDeleArtigo2}= useContext(AppContext);

  async function deleteArtigo2() {
    try {
      
      
      const response = await 
      // `https://boxer-relieved-impala.ngrok-free.app/allDiaconato/upArtigo`
      fetch(`${URLDeleteArtigo2}`, {
        headers: {
      ...
      >
        <AlertDialogOverlay>
          <AlertDialogContent>
            ...
            <AlertDialogBody>
              Tem certeza que deseja deletar esse Artigo do Aplicativo?
            </AlertDialogBody>
