...
export default function DialogDeleteNotice2({ id, Close }: Props) {
  const { isOpen, onOpen, onClose } = useDisclosure();
  const cancelRef = useRef();
  const toast = useToast()


  const {URLDeleteNotice2, setURLDeleteNotice2}= useContext(AppContext);

  async function deleteNotice2() {
    try {
      
      
      const response = await 
      // `https://boxer-relieved-impala.ngrok-free.app/allDiaconato/upArtigo`
      fetch(`${URLDeleteNotice2}`, {
        headers: {
